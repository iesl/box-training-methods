...
class TBox(Module):
    ...
    def scores(self, instance_boxes, label_boxes, intersection_temp, volume_temp):
        ...
        label_boxes:    (batch_size, num_{pos/neg}, 2, dim)
        """

        # boxes: (batch_size, num_{pos/neg}, 2 (u/v), 2 (min/-max), dim)
        label_boxes = label_boxes.unsqueeze(dim=2)
        instance_boxes = torch.broadcast_to(instance_boxes.unsqueeze(dim=1).unsqueeze(dim=1), label_boxes.shape)
        boxes = torch.cat([label_boxes, instance_boxes], dim=2)
