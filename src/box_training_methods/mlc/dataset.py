...
def edges_from_hierarchy_edge_list(
    edge_file: Union[
        Path, str
    ] = "/work/pi_mccallum_umass_edu/brozonoyer_umass_edu/box-training-methods/data/mesh/MeSH_parent_child_mapping_2020.txt",
    mesh=False,
) -> Tuple[LongTensor, LabelEncoder]:
    """
    ...
    edges = pd.read_csv(edge_file, sep=" ", header=None).to_numpy()[
        :, :2
    ]  # ignore line-final "{}"
    if mesh:
...
def name_id_mapping_from_file(
    name_id_file: Union[
        Path, str
    ] = "/work/pi_mccallum_umass_edu/brozonoyer_umass_edu/box-training-methods/data/mesh/MeSH_name_id_mapping_2020.txt"
) -> Dict:
...
class ARFFReader(object):
        ...
        **kwargs: Any,
        ...
        if labels_to_skip is None:
            self.labels_to_skip = [
                ...
                "GO0005575",
                "GO0008150",
            ]  # hardcode for HMLC datasets for now
    ...
    def _arff_dataset(
        ...
        # remove root
        to_take = np.logical_not(
            np.in1d(all_labels, self.labels_to_skip)
        )  # shape =(num_labels,),
...
class InstanceLabelsDataset(Dataset):
    ...
    def __attrs_post_init__(self):
        ...
        self.instance_feats = torch.nn.Embedding.from_pretrained(
            self.instance_feats, freeze=True
        )
    ...
    def prune_and_encode_labels_for_instances(self):
        ...
        for ls in self.labels:
            pruned_labels.append(
                self.label_encoder.transform(self.prune_labels_for_instance(ls))
            )
