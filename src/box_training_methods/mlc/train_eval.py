...
from .dataset import (
    ...
    BioASQInstanceLabelsIterDataset,
)
from box_training_methods.graph_modeling.dataset import (
    RandomNegativeEdges,
...
from box_training_methods.multilabel_classification.instance_encoder import (
    InstanceAsPointEncoder,
    InstanceAsBoxEncoder,
)
from box_training_methods.multilabel_classification.instance_scorers.instance_as_box_scorers.hard_box_scorer import (
    HardBoxScorer,
)


__all__ = ["setup_model", "setup_training_data", "EvalLooper"]


def setup_model(
    num_labels: int, instance_dim: int, device: Union[str, torch.device], **config
    ...
    if model_type == "gumbel_box":
        ...
        label_label_loss_func = BCEWithLogsNegativeSamplingLoss(
        ...
        box_model = TBox(num_labels, config["dim"], hard_box=True)
    ...
    instance_encoder = InstanceAsBoxEncoder(
        instance_dim=instance_dim, hidden_dim=64, output_dim=config["dim"]
    )
...
def setup_training_data(
    device: Union[str, torch.device], **config
) -> Tuple[
    GraphDataset, InstanceLabelsDataset, InstanceLabelsDataset, InstanceLabelsDataset
]:
    """
    ...
    taxonomy_edges, label_encoder = edges_from_hierarchy_edge_list(
        edge_file=hierarchy_edge_list_file
    )
    ...
    train_dataset = InstanceLabelsDataset(
        instance_feats=instance_feats_train,
        labels=labels_train,
        label_encoder=label_encoder,
    )
    dev_dataset = InstanceLabelsDataset(
        instance_feats=instance_feats_dev,
        labels=labels_dev,
        label_encoder=label_encoder,
    )
    test_dataset = InstanceLabelsDataset(
        instance_feats=instance_feats_test,
        labels=labels_test,
        label_encoder=label_encoder,
    )
...
def setup_mesh_training_data(device: Union[str, torch.device], **config):
    ...
    negative_sampler = MESHNegativeSampler()  # TODO add args
